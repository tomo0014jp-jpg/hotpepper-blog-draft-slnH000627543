...
- **`body` 内に絵文字を一切含めない（プレーンテキスト送信のため）。**
  - NG: ✨ 🌿 🌸 ☆ 🌟 ♪ ❤ 💕 😊 など Unicode の絵文字（U+1F300以降の絵文字、
    U+2700前後の記号絵文字）。Gmail のプレーンテキスト表示で文字化けします。
  - OK: `subject` 内の `【TEST】` `【ブログ下書き ...】` などの **括弧記号**（角括弧・隅付き括弧・
    山括弧などの一般的な日本語記号）。
  - OK: `body` 内の `────────────────────────`（罫線）・`■`・`▼`・`／` などの
    日本語標準記号（これらは絵文字ではないので可）。
  - OK: ハッシュタグ案（`#xxx`）— ハッシュタグ自体に絵文字を含めなければ問題なし。
