...
7. 全POSTが 200 で完了した場合のみ、履歴を反映する。
   - TESTモード：今日の3テーマを `data/test-topics.md` に追記。`data/past-topics.md` は変更しない。
   - PRODモード：今日の3テーマを `data/past-topics.md` に追記。
     可能なら main に直接 commit & push（メッセージ：`chore: append topics YYYY-MM-DD`）。
     直接コミット不可の場合は PR を作成し、翌日実行前までにマージが必要であることを
     見出しメッセージ内にも明記する。
