...
9. 生成したテーマ（タイトル・要旨）を、モードに応じたファイルへ追記する。
   ...
   - 追記後、**ローカルコミットまでで完了**とする（リモートへの `git push` は行わない）。
     詳細は §11 を参照。
...
## 11. 履歴ファイルの書き込み運用（push しない）

毎日の Routine 実行時、Claude が `data/test-topics.md`（PROD では `data/past-topics.md`）に
追記する処理は、**ローカルでの追記＋必要なら `git commit` まで**で完了とします。
**`git push` は行わないでください。**

### なぜ push しないか

- 重複チェック（§2 step 4-5）は **「実行時にローカルの `data/test-topics.md` を `Read` するだけ」**
  で成り立っており、リモートに同期されていなくても判定ロジック自体は機能する。
- リモート書き込み権限の管理（PAT, deploy key, GitHub App など）を Routine 環境に渡す
  必要がなくなり、機密情報の取扱面積を減らせる。
- push 失敗（権限エラー、ネットワーク不調、リモート競合など）でメール送信成功 +
  履歴非追記という **不整合状態**になるリスクを避けられる。

### 実装上の注意

- **`git push` 系のコマンドは実行しない。** `git push origin main` などをログに出さないこと。
- `git commit` 自体は行ってよい（Routine 環境内でローカル履歴として残す）が、
  必須ではない。ファイル更新さえ完了していれば、当該 Routine 実行内では十分。
- **GitHub 上のリポジトリは「テンプレ・ルールブックの正本」**としてのみ運用し、
  日々の生成履歴は GitHub に同期されないことを前提とする。
- もし将来「複数環境からの履歴を統合したい」「ヒトが生成履歴を GitHub 上で読みたい」
  という運用要件が出てきたら、その時点で本ルールを見直す。

### Claude Code Routines の挙動上の補足

- Anthropic の CCR（Cloud Coding Runtime）は、Routine ごとに毎回 fresh clone から
  作業を始める想定です。この場合、**前日のローカル追記は次回実行時には残らない**
  可能性があります（毎回 origin/main から clone される設計のため）。
- 重複判定は「リポジトリ origin/main にコミット済みのもの」＋「当該実行内で生成した3本」の
  範囲で行えば実用上は十分です。完全な日次累積を取りたい場合は、別途 `data/test-topics.md`
  をオフラインで手動更新するか、運用ルール自体を見直してください。
- いずれにしても、**Routine 側から push を強制する必要はなく、push が失敗しても
  当日のメール送信機能には影響しない**設計です。
